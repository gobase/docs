...
### パス
```
GET /v1/nft/offchain/market/{address}/tokens/{token_id}/buy/signature
```
...
https://api.gobase.io/v1/nft/offchain/market/{address}/tokens/{token_id}/buy/signature
```
...
```js
  ...
  marketContract.methods.buyToken(signature, nftContractAddress, tokenId).send(
    {
      from: account,
      value: wei,
    }, (err, tx_hash) => {
